...
function tableData(content: string) {
  return <td className="whitespace-nowrap px-4 py-2 font-medium text-gray-900">
    {content}
  </td>;
}

export default async function Page({ params }: { params: { owner: string, repo: string } }) {
        ...
        <div className="sm:flex sm:items-center sm:justify-between">
          <div className="text-center sm:text-left">
            <h1 className="text-2xl font-bold text-gray-900 sm:text-3xl">
              PRs for <code>{params.owner}/{params.repo}</code>
            </h1>
              ...
              <table className="min-w-full divide-y-2 divide-gray-200 bg-white text-sm">
                ...
                <tr>
                  {headings.map((heading, key) => (
                    <th key={key} className="whitespace-nowrap px-4 py-2 font-medium text-gray-900">
                      {ucFirst(heading)}
                    </th>))
                  }
                </tr>
